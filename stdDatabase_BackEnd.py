...
def searchData(StdID="", Firstname="", Surname="", DoB="", Age="", Gender="", Address="", Mobile=""):
    con=sqlite3.connect("student.db")
    cur = con.cursor()
    cur.execute("""SELECT * FROM student WHERE StdID=:stdid OR Firstname=:fname OR Surname=:sname OR DoB=:dob OR Age=:age OR Gender=:gender OR  Address=:address OR  Mobile=:mobile""", 
                {
                    'stdid': StdID, 
                    'fname': Firstname, 
                    'sname': Surname, 
...
def dataUpdate(id, StdID="", Firstname="", Surname="", DoB="", Age="", Gender="", Address="", Mobile=""):
    con=sqlite3.connect("student.db")
    cur = con.cursor()
    cur.execute("UPDATE student SET StdID=? OR Firstname=? OR Surname=? OR DoB=? OR Age=? OR Gender=? OR  Address=? OR  Mobile=? OR id=?", 
